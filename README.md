...
- ✅ **国内直连优化**：通过自定义域名 `cusxxin.cyou` 解决 DNS 污染。
...
- **核心功能**：定时抓取 RSS/网页 → AI 摘要 → 结构化存储 → 前端面板展示

![界面截图](./src/images/3.png)

- **技术栈**：Cloudflare Workers Cron + D1 + Workers AI + Vue 3
- **亮点**：无服务器定时自动化、流式摘要、零成本运营
- **在线演示**：[monitor.chusxxin.cyou](https://monitor.chusxxin.cyou)

## 📝 已知限制与改进方向

- Cloudflare 免费模型上下文仅 8K，超长对话需实现滑动窗口裁剪。
- 停止生成后无法从断点续传（受限于无状态推理 API）。
- 未来计划：引入 **Prompt 模板** 支持多角色切换。
